...
class KandinskyPriorPipeline(DiffusionPipeline):
    ...
    def interpolate(
        ...
        device=None,
        search_k: bool = False,
        top_k_image : Optional[torch.Tensor] = None,
        top_k_w : Optional[List[float]] = None
        ...
        if search_k:
            print("add top k image")
            if len(top_k_image) != len(top_k_w):
                raise ValueError(f"`top_k_image` contains {len(top_k_image)} items and `top_k_w` contains {len(top_k_w)} items - they should be lists of same length")
            for cond, weight in zip(top_k_image, top_k_w):
                image_embeddings.append((cond * weight).unsqueeze(0))
            print(len(image_embeddings))
                

        #image_embeddings = [torch.tensor(emb) if isinstance(emb, np.ndarray) else emb for emb in image_embeddings]
        # for e in image_embeddings:
        #    print(e.shape) #(1, 768)
        image_emb = torch.cat(image_embeddings).sum(dim=0, keepdim=True)
        print(image_emb.shape) #(1, 768)
